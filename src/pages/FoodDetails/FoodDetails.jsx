...
//     if (diffDays <= 7) return { status: 'caution', color: 'from-primary to-indigo-500', days: diffDays };
...
//             </span>
//           )}
//           <span className="bg-gradient-to-r from-primary to-indigo-500 text-white px-3 py-1 rounded-lg text-sm font-semibold">
//             {hours}h
//           </span>
...
//         >
//           <div className="w-20 h-20 bg-gradient-to-br from-primary to-primary rounded-full flex items-center justify-center mx-auto mb-6 shadow-2xl">
//             <motion.div
//               animate={{ rotate: 360 }}
...
//             whileTap={{ scale: 0.98 }}
//           >
//             <ArrowLeft className="w-5 h-5 text-slate-600 dark:text-slate-300 group-hover:text-primary dark:group-hover:text-primary transition-colors" />
//             <span className="font-semibold text-slate-700 dark:text-slate-200 group-hover:text-primary dark:group-hover:text-primary transition-colors">
//               Back to List
//             </span>
//           </motion.button>
//         </motion.div>
...
//                     {food.title}
//                   </h1>
//                   <div className="w-16 h-1 bg-gradient-to-r from-primary to-primary rounded-full"></div>
//                 </div>
...
//                         <Tag className="w-5 h-5 text-primary dark:text-primary" />
//                       </div>
...
//             >
//               {/* Add Note Section */}
//               <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 p-6 rounded-2xl border border-blue-200/50 dark:border-primary/50">
//                 <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-4 flex items-center gap-3">
//                   <div className="p-2 bg-blue-100 dark:bg-blue-900/50 rounded-lg">
//                     <Edit3 className="w-5 h-5 text-primary dark:text-primary" />
//                   </div>
//                   Add New Note
//                   {isOwner && (
...
//                       isOwner 
//                         ? 'border-blue-200 dark:border-primary focus:border-primary dark:focus:border-primary focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900/50' 
...
//                         ? 'bg-gradient-to-r from-primary to-primary hover:from-primary hover:to-primary text-white shadow-lg hover:shadow-xl'
...
const FoodDetails = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        setFood(res.data);
        setIsOwner(user && res.data.userEmail === user.email);
      })
      .catch((err) => {
        console.error("Error fetching food:", err.message);
        Swal.fire({
  ...
  const handleAddNote = async () => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_API_URL}/foods/${id}/notes`,
        { note: noteText, userEmail: user.email },
        { withCredentials: true }
      );
  ...
  const getExpiryStatus = (expiryDate) => {
    ...
    if (days < 0) return { status: "expired", color: "bg-primary" };
    if (days <= 1) return { status: "critical", color: "bg-primary" };
    if (days <= 3) return { status: "warning", color: "bg-primary" };
    if (days <= 7) return { status: "caution", color: "bg-primary" };
    return { status: "fresh", color: "bg-green-600" };
  };
    ...
    return (
      <div className="flex items-center gap-2 text-gray-800 dark:text-gray-200">
        ...
        <div className="flex gap-2">
          <span className="bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-100 px-2 py-1 rounded-md text-sm font-medium">
            {days}d
          </span>
          <span className="bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-100 px-2 py-1 rounded-md text-sm font-medium">
            {hours}h
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-900 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <motion.button
          onClick={() => navigate(-1)}
          className="mb-8 inline-flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border rounded-lg shadow hover:shadow-md text-slate-700 dark:text-slate-200"
        ...
        {/* Card */}
        <motion.div
          className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl border dark:border-slate-700 overflow-hidden"
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
        >
          <div className={`h-2 ${expiryStatus.color}`}></div>
          <div className="p-8">
            <div className="grid lg:grid-cols-2 gap-8">
              ...
              <div className="relative">
                ...
                />
                <div
                  className={`absolute top-3 right-3 px-3 py-1 text-sm text-white rounded-full ${expiryStatus.color}`}
                >
                  {expiryStatus.status.toUpperCase()}
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-gray-50 dark:bg-slate-700 rounded-lg border">
                  <h3 className="font-bold mb-2 flex items-center gap-2">
                    <Clock className="w-5 h-5 text-amber-500" /> Time Remaining
                  </h3>
                  <Countdown date={new Date(food.expiryDate)} renderer={countdownRenderer} />
                </div>
              </div>
            ...
            <div className="mt-10 space-y-6">
              {/* Add Note */}
              <div className="p-6 bg-gray-50 dark:bg-slate-700 rounded-xl border">
                <h3 className="font-bold mb-4 flex items-center gap-2">
                  <Edit3 className="w-5 h-5 text-primary" /> Add Note
                </h3>
                <textarea
                  disabled={!isOwner}
                  value={noteText}
                  ...
                  className="w-full p-3 border rounded-lg dark:bg-slate-800 dark:border-slate-600"
                />
                <button
                  onClick={handleAddNote}
                  disabled={!isOwner || !noteText.trim()}
              ...
              <div className="p-6 bg-gray-50 dark:bg-slate-700 rounded-xl border">
                <h3 className="font-bold mb-4 flex items-center gap-2">
                  <MessageCircle className="w-5 h-5" /> Previous Notes
                </h3>
                {food.notes?.length ? (
                  <div className="space-y-3 max-h-96 overflow-y-auto">
                    {food.notes.map((note, i) => (
                      ...
                      >
                        <p className="text-gray-700 dark:text-gray-200 mb-2">{note.note}</p>
                  ...
                  <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                    <MessageSquarePlus className="w-8 h-8 mx-auto mb-3" />
                    No notes yet.
                  </div>
                )}
              </div>
